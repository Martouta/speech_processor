...
class TestDownloadTiktokVideo:
    def teardown_method(self):
        for filename in tiktok_ids:
            path = f"{os.getcwd()}/resources/videos/test/download-tiktok-{filename}.mp4"
    ...
    def test_download_tiktok_video_simple(self):
        path_tuple = ('resources/videos/test', f"download-tiktok-{tiktok_ids[0]}.mp4")
    ...
    def test_download_tiktok_video_with_threads(self):
        with concurrent.futures.ThreadPoolExecutor(len(tiktok_ids)) as executor:
            for filename in tiktok_ids:
                filepath_tuple = (f"{os.getcwd()}/resources/videos/test", f"download-tiktok-{filename}.mp4")
                app.download_tiktok_video(filename, filepath_tuple)
        for filename in tiktok_ids:
            path = f"{os.getcwd()}/resources/videos/test/download-tiktok-{filename}.mp4"
