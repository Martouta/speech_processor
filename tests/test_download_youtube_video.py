...
class TestDownloadYoutubeVideo:
    def teardown_method(self):
        for filename in youtube_ids:
            path = f"{os.getcwd()}/resources/videos/test/download-youtube-{filename}.mp4"
    ...
    def test_download_youtube_video_simple(self):
        path_tuple = ('resources/videos/test', f"download-youtube-{youtube_ids[0]}.mp4")
    ...
    def test_download_youtube_video_with_threads(self):
        with concurrent.futures.ThreadPoolExecutor(len(youtube_ids)) as executor:
            for filename in youtube_ids:
                app.download_youtube_video(filename, (f"{os.getcwd()}/resources/videos/test", f"download-youtube-{filename}.mp4"))
        for filename in youtube_ids:
            path = f"{os.getcwd()}/resources/videos/test/download-youtube-{filename}.mp4"
